<template>
  <div class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-background-dark">
      ...
      <div class="flex items-center justify-between gap-3">
        ...
        <div class="flex shrink-0 flex-col gap-2 sm:flex-row">
          ...
          <button
            ...
            @click="agentEnabled ? turnOffAgent() : openBindModal()"
          >
    ...
    <div
      v-if="bindModalOpen"
      class="fixed inset-0 z-[1100] flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-sm"
      role="dialog"
      aria-modal="true"
      :aria-label="t('agent_bindTitle')"
    ...
    >
      <div class="w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900">
        <div class="border-b border-slate-200 bg-slate-50/85 px-5 py-4 dark:border-slate-700 dark:bg-slate-800/65">
          <div class="flex items-start justify-between gap-4">
            <div>
              <p class="text-[11px] font-bold uppercase tracking-[0.22em] text-primary">{{ t('agent_bindEyebrow') }}</p>
              <h3 class="mt-1 text-lg font-semibold text-slate-900 dark:text-slate-100">{{ t('agent_bindTitle') }}</h3>
              <p class="mt-1 text-sm leading-6 text-slate-500 dark:text-slate-400">{{ t('agent_bindDesc') }}</p>
            </div>
            <button
              type="button"
              class="rounded-xl p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-slate-800 dark:hover:text-slate-200"
        ...
        <div class="space-y-4 p-5">
          <div class="mx-auto flex size-64 items-center justify-center rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700">
            <img
              v-if="bindSession?.qr_data_url"
              class="h-full w-full object-contain"
              :src="bindSession.qr_data_url"
              :alt="t('agent_bindQrAlt')"
            />
            <div v-else class="text-center text-sm text-slate-400">{{ t('agent_generatingQr') }}</div>
          </div>

          <div class="rounded-xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-700 dark:border-sky-500/30 dark:bg-sky-500/10 dark:text-sky-200">
            {{ bindMessage }}
          </div>
          ...
          <div class="flex gap-3">
            <button
              type="button"
              class="inline-flex h-11 flex-1 items-center justify-center rounded-xl border border-slate-200 px-4 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-100 dark:hover:bg-slate-800"
              @click="closeBindModal"
            >
              {{ t('agent_cancel') }}
            </button>
            <button
              type="button"
              class="inline-flex h-11 flex-1 items-center justify-center rounded-xl bg-primary px-4 text-sm font-semibold text-white transition hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-60"
              :disabled="binding"
              @click="startBinding"
            >
              {{ bindSession ? t('agent_regenerateQr') : t('agent_generateQr') }}
            </button>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    statusLabel() {
      ...
      if (this.status?.status === 'pending_bind') return this.t('agent_pending');
    ...
    statusTitle() {
      ...
      if (this.status?.status === 'pending_bind') return this.t('agent_pendingTitle');
    ...
    statusBadgeClass() {
      ...
      if (this.status?.status === 'pending_bind') return 'bg-sky-100 text-sky-700 dark:bg-sky-500/15 dark:text-sky-300';
    ...
    statusPanelClass() {
      ...
      if (this.status?.status === 'pending_bind') return 'border-sky-200 bg-sky-50/70 dark:border-sky-500/30 dark:bg-sky-500/10';
    ...
    bindMessage() {
      return this.bindStatus?.message || this.bindSession?.message || this.t('agent_bindWaiting');
    },
    agentServerLabel() {
  ...
  methods: {
    ...
    async startBinding() {
      ...
      this.stopBindPolling();
      try {
        this.bindSession = await startAgentBinding();
        this.startBindPolling();
      } catch (err) {
        this.setFeedback(err instanceof Error ? err.message : this.t('agent_bindFailed'), 'error');
      } finally {
        this.binding = false;
      }
    },
    startBindPolling() {
      if (!this.bindSession?.session_id) return;
      this.pollBindStatus();
      this.bindPollTimer = window.setInterval(this.pollBindStatus, 2000);
    },
    stopBindPolling() {
      if (this.bindPollTimer !== null) {
        window.clearInterval(this.bindPollTimer);
    ...
    async pollBindStatus() {
      if (!this.bindSession?.session_id) return;
      try {
        const status = await getAgentBindingStatus(this.bindSession.session_id);
        this.bindStatus = status;
        if (status.bound) {
          this.stopBindPolling();
          await this.refreshStatus();
          this.setFeedback(this.t('agent_bindSuccess'), 'success');
          window.setTimeout(() => {
            this.bindModalOpen = false;
          }, 900);
        }
        ...
        this.stopBindPolling();
        this.setFeedback(err instanceof Error ? err.message : this.t('agent_bindStatusFailed'), 'error');
      }
    },
    async turnOffAgent() {
      this.loading = true;
