...
func TestAuthServiceRestoreSession_IsPureSnapshotRead(t *testing.T) {
	...
	var remoteRequests int
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		remoteRequests++
		http.Error(w, "session read must not call remote auth", http.StatusInternalServerError)
	}))
	defer server.Close()
	...
	payload, ok := result["data"].(SessionSnapshotPayload)
	if !ok || payload.State != "active" || payload.User == nil || payload.User.Username != "stale-user" {
		t.Fatalf("snapshot payload = %#v", result["data"])
	}
	after := authority.Snapshot()
	if after.Revision != before.Revision || after.Generation != before.Generation {
		t.Fatalf("session read mutated authority: before=%+v after=%+v", before, after)
	}
	if remoteRequests != 0 {
		t.Fatalf("session read made %d remote requests", remoteRequests)
	}
}
