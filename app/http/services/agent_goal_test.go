...
func TestAttachGoalReportFallbackInfersCompletedFromOutput(t *testing.T) {
	var terminal map[string]interface{}
	// Model a CHECKED task: optimism is safe because server-side verification
	// backstops a false task_completed. goalRequiredCheckCount>0 ⇒ hasChecks.
	emitter := newAgentAIRunEmitter(agentAIRun{
		runID: "run-1", goalIdentity: testGoalIdentity(), goalRequiredCheckCount: 1,
	}, func(value interface{}) error {
		terminal = value.(map[string]interface{})
		return nil
	})
	...
	report := terminal["goal_report"].(map[string]interface{})
	if report["outcome"] != "task_completed" {
		t.Fatalf("outcome = %v, want task_completed (checked-task fallback should be optimistic)", report["outcome"])
	}
...
// Zero-check audit/research task: no marker + non-empty output must NOT infer
// task_completed — there is no verification backstop (manifest.ts:280), so the
// optimism would manufacture a false completion. Surfaces as no_progress for
// the sign-off gate to review. Regression guard for the false-completion bug.
func TestAttachGoalReportFallbackZeroCheckDoesNotClaimCompletion(t *testing.T) {
	var terminal map[string]interface{}
	emitter := newAgentAIRunEmitter(agentAIRun{
		runID: "run-1", goalIdentity: testGoalIdentity(), // goalRequiredCheckCount=0
	}, func(value interface{}) error {
		terminal = value.(map[string]interface{})
		return nil
	})
	_ = emitter.emit(map[string]interface{}{"type": models.AgentEventAIDelta, "delta": "Audited the module; notes above."})
	_ = emitter.emit(map[string]interface{}{"type": models.AgentEventAIDone})
	report := terminal["goal_report"].(map[string]interface{})
	if report["outcome"] == "task_completed" {
		t.Fatalf("zero-check task falsely marked task_completed: %#v", report)
	}
	if report["completion_proposed"] == true {
		t.Fatalf("zero-check task must not propose completion without a marker: %#v", report)
	}
}
