...
func (e *agentAIRunEmitter) attachGoalReport(payload map[string]interface{}) {
	...
	// hasChecks gates the optimistic task_completed branch — see inferGoalReportFromOutput.
	report := inferGoalReportFromOutput(capturedOutput, remoteString(payload, "type"), e.goalRequiredCheckCount > 0)
...
// goal_context envelope (the same one serializeGoalContext renders). Used by
// attachGoalReport to decide whether an optimistic completion is safe. Returns
// 0 for any absent/malformed envelope — the conservative (no-checks) answer.
func goalRequiredCheckCountFromContext(raw interface{}) int {
	...
	checks, ok := task["requiredChecks"].([]interface{})
	if !ok {
		return 0
	}
	return len(checks)
...
//
// hasChecks gates the optimistic branch: a zero-check audit/research task has
// NO server-side verification backstop (manifest.ts:280 — it completes purely
// on this report), so inferring task_completed from mere non-empty output
// manufactures false completions. Only infer completion when checks exist.
func inferGoalReportFromOutput(capturedOutput, terminalEventType string, hasChecks bool) map[string]interface{} {
	...
	// 乐观：有输出且无明显错误。
	if hasChecks {
		// 仅对有机器检查的 task 才乐观判 task_completed —— server 端验证会兜住
		// 假完成。这是历史修复：之前直接 goal_report_missing 让很多成功 task 失败。
		return map[string]interface{}{
			"schema_version":      1,
	...
	// 零检查的 audit/research task：无 server 端验证兜底（manifest.ts:280 —— 它仅
	// 靠本报告完成），不得从"输出非空"乐观判 task_completed，否则假完成。降级
	// no_progress，把输出交到签署闸由用户判定。
	return map[string]interface{}{
		"schema_version":      1,
		"outcome":             "no_progress",
		"summary":             "Provider produced output without a structured report; this task has no machine checks, so completion cannot be inferred — review the output and accept at the sign-off gate if done. Output tail: " + clipSummary(tail),
		"blocker_code":        "goal_report_missing",
		"evidence_refs":       []interface{}{},
		"completion_proposed": false,
	}
...
func handleAgentGoalPlan(msg map[string]interface{}, writeJSON func(interface{}) error) {
	...
	callServer := func(turn int, messages []map[string]interface{}) (map[string]interface{}, string, error) {
		...
		logGoalPlanTurnStart(goalID, turn, messages)
		...
		for {
			...
			case response := <-responseCh:
				...
				assistant, assistantResponseID, assistantErr := extractGoalPlanAssistant(response)
				if assistantErr == nil {
					logger.Info(fmt.Sprintf("[goal.plan] goal=%s turn=%d model wants: %s", goalID, turn, describeGoalPlanAssistantAction(assistant)))
				}
				return assistant, assistantResponseID, assistantErr
			case <-progress.C:
				emitGoalPlanPhase(writeJSON, requestID, "planning", turn)
			case <-timer.C:
				return nil, "", errors.New("planner_service_transport_timeout")
	...
	// Honor the server-provided turn budget (planner_max_turns); fall back to
	// the local default when the server omits it.
	effectiveMaxTurns := goalPlannerMaxTurns
	if pt := remoteInt(msg, "planner_max_turns", 0); pt > 0 {
		if pt > 60 {
			pt = 60
		}
		effectiveMaxTurns = pt
	}
	// askUser closure: sends goal.plan.ask_user via WS, waits for the user's
	// answer on responseCh (no timeout — the user can take as long as needed),
	// freezes the planner deadline during the wait (user think-time doesn't eat
	// the AI budget), and returns the answer text. Planning is paused while
	// waiting; the server shows the question on the phone via the approval system.
	askUser := func(question string, options []interface{}) (string, error) {
		if err := writeJSON(map[string]interface{}{
			"type":       "goal.plan.ask_user",
			"request_id": requestID,
			"goal_id":    goalID,
			"question":   question,
			"options":    options,
		...
		emitGoalPlanPhase(writeJSON, requestID, "awaiting_user_input", 0)
		logger.Info(fmt.Sprintf("[goal.plan] goal=%s ask_user: %s", goalID, snippetForGoalPlan(question, 120)))
		askUserStart := time.Now()
		for {
			select {
			case response := <-responseCh:
				answer := remoteString(response, "_ask_user_answer")
				// Freeze: extend deadline by the ask_user wait so user think-time
				// doesn't eat the planner's wall-clock budget.
				deadline = deadline.Add(time.Since(askUserStart))
				return answer, nil
			case <-progress.C:
				emitGoalPlanPhase(writeJSON, requestID, "awaiting_user_input", 0)
			}
...
// goalPlanAskUserCaller is injected into runGoalPlanLoop so the loop can ask
// the user a question during planning (via WS → server → approval → phone →
// user → response → WS back). The closure handles all WS communication; the
// loop just gets the answer string back. Returns the user's answer text.
type goalPlanAskUserCaller func(question string, options []interface{}) (string, error)

type goalPlanLoopInput struct {
	projectRoot string
	evidence    map[string]interface{}
	callServer  goalPlanTurnCaller
	askUser     goalPlanAskUserCaller
	maxTurns    int
}
...
// logGoalPlanTurnStart emits the trailing tool result from the previous turn so
// the multi-round planning interaction is observable in the agent log (what the
// model read/grepped and got back each turn). The production loop otherwise
// holds the conversation in memory and discards it after planning, leaving the
// interaction invisible to operators.
func logGoalPlanTurnStart(goalID string, turn int, messages []map[string]interface{}) {
	...
	logger.Info(fmt.Sprintf("[goal.plan] goal=%s turn=%d prev tool result: %s", goalID, turn, snippetForGoalPlan(fmt.Sprintf("%v", last["content"]), 200)))
}

// describeGoalPlanAssistantAction renders what the model did this turn — tool
// calls with their path/pattern arguments, or a content snippet — the readable
// form of "how is the planner exploring", persisted for post-hoc inspection.
func describeGoalPlanAssistantAction(msg map[string]interface{}) string {
	if raw, ok := msg["tool_calls"].([]interface{}); ok && len(raw) > 0 {
		...
		for _, c := range raw {
			...
			parts = append(parts, fmt.Sprintf("%s(%s)", name, snippetForGoalPlan(interesting, 80)))
		}
		return "tools=[" + strings.Join(parts, ", ") + "]"
	}
	if c, _ := msg["content"].(string); strings.TrimSpace(c) != "" {
		return "content=" + snippetForGoalPlan(c, 120)
	}
	return "(no tool_calls, no content)"
}

func snippetForGoalPlan(s string, n int) string {
...
func runGoalPlanLoop(input goalPlanLoopInput) (map[string]interface{}, string, error) {
	...
	for turn := 1; turn <= input.maxTurns; turn++ {
		...
		for _, raw := range toolCalls {
			...
			if name == "ask_user" && input.askUser != nil {
				var args struct {
					Question string        `json:"question"`
					Options  []interface{} `json:"options"`
				}
				_ = json.Unmarshal([]byte(arguments), &args)
				answer, askErr := input.askUser(args.Question, args.Options)
				if askErr != nil {
					answer = "Error asking user: " + askErr.Error()
				}
				if answer == "" {
					answer = "User did not provide a response."
				}
				logger.Info(fmt.Sprintf("[goal.plan] ask_user answered: %s", snippetForGoalPlan(answer, 100)))
				messages = append(messages, map[string]interface{}{
...
func goalWorkspaceFingerprint(root string) (string, error) {
	...
	err := filepath.WalkDir(root, func(path string, entry os.DirEntry, walkErr error) error {
		...
		if shouldSkipGoalFingerprintFile(entry.Name()) {
			return nil
		}
	...
	for _, path := range files {
		...
		if err != nil {
			// Tolerate files that vanished between the walk and this lstat
			// (transient .tmp/.swp files on an actively-edited workspace) —
			// skip them instead of failing the whole fingerprint (and the goal).
...
// shouldSkipGoalFingerprintFile drops transient files an actively-edited workspace
// produces (atomic-write temps like `foo.vue.tmp.<pid>.<hash>`, editor swaps,
// lock files, .DS_Store) so they neither destabilize the fingerprint nor trip a
// vanishing-file lstat race during the walk→lstat gap. Reported in prod as
// `lstat …/NamespaceOverview.vue.tmp.933022.…: no such file or directory`.
func shouldSkipGoalFingerprintFile(name string) bool {
	if strings.Contains(name, ".tmp.") || strings.HasSuffix(name, ".tmp") {
		return true
	}
	switch {
	case strings.HasSuffix(name, ".swp"), strings.HasSuffix(name, ".swo"), strings.HasSuffix(name, "~"):
		return true
