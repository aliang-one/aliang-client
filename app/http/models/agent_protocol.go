	...
	return AgentProtocolContract{
		...
		HTTP: []AgentProtocolHTTPRoute{
			...
			},
			{
				...
				RequestFields:  []string{"device_id", "status", "unique_code", "device_name", "platform", "agent_version", "capabilities", "tools", "history", "projects", "vibe_sessions", "authorized_directories", "started_at", "collected_at"},
		...
		WebSocket: AgentProtocolWebSocket{
			...
			ClientSends: []AgentProtocolEvent{
				{Type: AgentEventHello, Required: []string{"type", "device_id", "unique_code", "protocol_version", "device_name", "platform", "agent_version", "capabilities", "tools", "history", "projects", "vibe_sessions", "started_at"}, Optional: []string{"host", "authorized_directories", "collected_at"}},
				{Type: AgentEventHeartbeat, Required: []string{"type", "device_id", "ts"}},
			...
			ServerSends: []AgentProtocolEvent{
				...
				{Type: AgentEventAISessionDetail, Required: []string{"type", "request_id"}, Optional: []string{"session_id", "source_session_id", "project_path"}, Emits: []string{AgentEventAISessionDetailResult, AgentEventFileError}},
			},
		},
